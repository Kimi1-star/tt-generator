...
Layout: A4 portrait, no gridlines, fits 1 page wide.
"""
...
def amount_to_words(amount: float) -> str:
    ...
    def _full(n):
        ...
        parts = []
        if n >= 1_000_000:
            parts.append(_below_thousand(n // 1_000_000) + ' MILLION'); n %= 1_000_000
        if n >= 1_000:
            parts.append(_below_thousand(n // 1_000) + ' THOUSAND'); n %= 1_000
...
# ── A4 page setup (call after all rows are written) ─────────────────────────
def _setup_a4(ws, last_row: int, ncols: int = 6):
    col_letter = 'ABCDEFG'[ncols - 1]
    ws.print_area          = f'A1:{col_letter}{last_row}'
    ws.page_setup.paperSize    = 9           # A4
    ws.page_setup.orientation  = 'portrait'
    ws.page_setup.fitToPage    = True
    ws.page_setup.fitToWidth   = 1
    ws.page_setup.fitToHeight  = 99          # unconstrained height
    ws.page_margins = PageMargins(
        left=0.5, right=0.5, top=0.6, bottom=0.6,
        header=0.2, footer=0.2)
    ws.sheet_view.showGridLines = False      # no gridlines / 无虚线


# ── CI Sheet ─────────────────────────────────────────────────────────────────
# 6 columns A–F, total width ≈ 76 units → fits A4 portrait (173 mm usable)
# A=24  B=9  C=10  D=10  E=10  F=13
def _build_ci(ws, d):
    for col, w in zip('ABCDEF', [24, 9, 10, 10, 10, 13]):
    ...
    if d.get('buyer_address'): buyer_lines.append('ADD: ' + d['buyer_address'])
    if d.get('buyer_ref'):     buyer_lines.append(d['buyer_ref'])
    _cell(ws, r, 1, '\n'.join(buyer_lines))
    _cell(ws, r, 6, f"DATE: {d['invoice_date']}", h_align='right')
    _h(ws, r, 50); r += 1

    _cell(ws, r, 6, f"INVOICE NO.: {d['invoice_no']}", h_align='right'); r += 1

    r += 3  # blank rows

    # Description
    _merge(ws, r, 1, r, 6)
    _cell(ws, r, 1, 'DESCRIPTION OF GOODS & SERVICES', bold=True); r += 1

    _merge(ws, r, 1, r, 6)
    _cell(ws, r, 1, f"COMMODITY: {d['commodity']}"); r += 1

    _merge(ws, r, 1, r, 6)
    _cell(ws, r, 1, f"SALES CONTRACT NO.: {d['contract_no']}"); r += 1

    _merge(ws, r, 1, r, 6)
    _cell(ws, r, 1, f"COUNTRY OF ORIGIN: {d.get('country_of_origin', 'CHINA')}"); r += 1

    _merge(ws, r, 1, r, 6)
    _cell(ws, r, 1, f"PRICE TERMS: {d['price_terms']}"); r += 1

    r += 1  # blank

    # Table header rows
    #  col: A=blank  B=GRADE  C=SIZE  D=QUANTITY  E=UNIT PRICE  F=AMOUNT
    ...
    for g in d['grades']:
        ...
        for col, (v, f) in enumerate(zip(row_vals, fmts), 1):
            _cell(ws, r, col, v, h_align='right' if col >= 4 else 'center',
                  border=True, fmt=f)
        r += 1

    # TOTAL row — label in col B (under GRADE header)
    total_vals = ['', 'TOTAL', '', total_qty, '', total_amt]
    total_fmts = [None, None, None, '#,##0.000', None, '#,##0.00']
    for col, (v, f) in enumerate(zip(total_vals, total_fmts), 1):
        _cell(ws, r, col, v, bold=True,
              h_align='right' if col in (4, 6) else 'center',
              border=True, fmt=f)
    r += 1
    ...
    if d.get('has_prepayment') and d.get('prepayment'):
        ...
        _cell(ws, r, 6, prepayment, h_align='right', fmt='#,##0.00'); r += 1

        _cell(ws, r, 1, 'BALANCE', bold=True)
        _cell(ws, r, 6, balance, bold=True, h_align='right', fmt='#,##0.00'); r += 1
    else:
    ...
    if d.get('has_prepayment') and d.get('prepayment'):
        r += 1  # blank
        _merge(ws, r, 1, r, 6); _cell(ws, r, 1, f'BENEFICIARY: {BENEFICIARY}'); r += 1
        _merge(ws, r, 1, r, 6); _cell(ws, r, 1, f'ADD: {COMPANY_ADDR}'); r += 1
        for lbl, val in [('BANK NAME', BANK_NAME), ('BANK ADDRESS', BANK_ADDR),
                         ('SWIFT CODE', SWIFT_CODE), ('ACCOUNT NO.', ACCOUNT_NO)]:
            _merge(ws, r, 1, r, 6)
            _cell(ws, r, 1, f'{lbl}:{val}' if lbl == 'SWIFT CODE' else f'{lbl}: {val}')
            r += 1
        _cell(ws, r, 1, 'FOB VALUE: USD')
        _cell(ws, r, 3, fob, h_align='right', fmt='#,##0.00'); r += 1
        _cell(ws, r, 1, 'FREIGHT AMOUNT: USD')
        _cell(ws, r, 3, freight, h_align='right', fmt='#,##0.00'); r += 1
    else:
        ...
        _merge(ws, r, 1, r, 6); _cell(ws, r, 1, pay_txt)
        _h(ws, r, 35); r += 1
        _cell(ws, r, 1, 'FOB VALUE: USD')
        _cell(ws, r, 2, fob, h_align='right', fmt='#,##0.00'); r += 1
        _cell(ws, r, 1, 'FREIGHT AMOUNT: USD')
        _cell(ws, r, 2, freight, h_align='right', fmt='#,##0.00'); r += 2  # blank
        _merge(ws, r, 1, r, 6); _cell(ws, r, 1, f'BENEFICIARY: {BENEFICIARY}'); r += 1
        _merge(ws, r, 1, r, 6); _cell(ws, r, 1, f'ADD: {COMPANY_ADDR}'); r += 1
        for lbl, val in [('BANK NAME', BANK_NAME), ('BANK ADDRESS', BANK_ADDR),
                         ('SWIFT CODE', SWIFT_CODE), ('ACCOUNT NO.', ACCOUNT_NO)]:
    ...
    r += 2  # blank before signature
    _cell(ws, r, 4, COMPANY_NAME, bold=True, h_align='center')

    _setup_a4(ws, r, ncols=6)


# ── PL Sheet ─────────────────────────────────────────────────────────────────
# A=GRADE  B=SIZE  C=NUMBER OF COILS  D=NET WEIGHT  E=GROSS WEIGHT  (5 cols)
# Widths: A=22  B=12  C=13  D=14  E=14  → total=75
def _build_pl(ws, d):
    ...
    _cell(ws, r, 5, 'ORIGINAL', bold=True, h_align='right'); r += 1
    _cell(ws, r, 1, 'TO:', bold=True); r += 1
    ...
    if d.get('buyer_ref'):     buyer_lines.append(d['buyer_ref'])
    _cell(ws, r, 1, '\n'.join(buyer_lines))
    _cell(ws, r, 5, f"DATE: {d['invoice_date']}", h_align='right')
    _h(ws, r, 50); r += 1

    _cell(ws, r, 5, f"INVOICE NO.: {d['invoice_no']}", h_align='right'); r += 1
    ...
    for g in d['grades']:
        ...
        for col, (v, f) in enumerate(
                zip([g['grade'], str(g['size']), coils, net, gross],
                    [None, None, '#,##0', '#,##0.000', '#,##0.000']), 1):
            _cell(ws, r, col, v, h_align='right' if col >= 3 else 'center',
                  border=True, fmt=f)
        r += 1

    # TOTAL — label in col A (under GRADE)
    for col, (v, f) in enumerate(
            [('TOTAL', None), ('', None), (total_coils, '#,##0'),
             (total_net, '#,##0.000'), (total_gross, '#,##0.000')], 1):
        _cell(ws, r, col, v, bold=True,
              h_align='right' if col >= 3 else 'center',
              border=True, fmt=f)
    r += 1
...
# Widths: A=32  B=18  C=14  D=12  → total=76
def _build_sa(ws, d):
    ...
    _cell(ws, r, 4, 'ORIGINAL', bold=True, h_align='right'); r += 1
    _cell(ws, r, 1, 'TO:', bold=True); r += 1
    ...
    if d.get('buyer_ref'):     buyer_lines.append(d['buyer_ref'])
    _cell(ws, r, 1, '\n'.join(buyer_lines))
    _cell(ws, r, 4, f"DATE: {d['invoice_date']}", h_align='right')
    _h(ws, r, 50); r += 1

    _cell(ws, r, 4, f"INVOICE NO.: {d['invoice_no']}", h_align='right'); r += 1

    r += 3

    _merge(ws, r, 1, r, 4)
    _cell(ws, r, 1, 'SHIPMENT DETAILS', bold=True); r += 1
    _merge(ws, r, 1, r, 4)
    _cell(ws, r, 1, f"COMMODITY: {d['commodity']}"); r += 1
    ...
    _cell(ws, r, 2, total_qty, h_align='right', fmt='#,##0.000'); r += 1

    _cell(ws, r, 1, 'INVOICE VALUE: USD')
    _cell(ws, r, 2, total_amt, h_align='right', fmt='#,##0.00'); r += 1

    _cell(ws, r, 1, f"NAME OF VESSEL: {d.get('vessel', '')}"); r += 1
    _cell(ws, r, 1, f"PORT OF LOADING: {d.get('loading_port', '')}"); r += 1

    eta_label = d.get('eta_etd_label', 'ETA').upper()
    _cell(ws, r, 1, f"{eta_label}: {d.get('eta_etd_date', '')}"); r += 1

    r += 15
    _cell(ws, r, 2, COMPANY_NAME, bold=True, h_align='center')
